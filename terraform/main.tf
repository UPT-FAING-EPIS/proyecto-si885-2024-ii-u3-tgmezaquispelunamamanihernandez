...
resource "aws_glue_catalog_table" "csv_table" {
  ...
  storage_descriptor {
    ...
    # Puedes añadir más columnas si tu archivo CSV las requiere
  }
}

# Crear un Rol IAM para Glue
...
# Política IAM para Glue (Permisos de S3 y Glue)
...
# Adjuntar la política de Glue al rol IAM
...
resource "aws_glue_crawler" "csv_crawler" {
  ...
  # No especificamos clasificadores para que Glue detecte el tipo de archivo automáticamente
  classifiers = []  # Glue detecta el tipo de archivo automáticamente sin necesidad de especificar clasificadores

  table_prefix = "csv_"  # Prefijo para las tablas que se crean

  # Eliminar "CrawlerOutput" y "partitionKeys", que no son necesarios
  configuration = jsonencode({
    "Version" = 1.0  # Solo incluimos la versión numérica
  })
}
